...
`Section`'s inner container applies a uniform `gap-6` (24px) between every
direct child. That's the right rhythm between text elements (title, lead,
cards), but an image (`ImageFull`, `ImageGrid2`, `EmbedFull`, `VideoFull`,
`ImageCarousel`) sitting directly between paragraphs/callouts needs more
breathing room: **32px**, not 24px. Don't edit `CaseStudyPrimitives.tsx`'s
shared `gap-6` to get this — that's used by every case study on the site,
so changing it there silently reflows all of them. Instead, wrap the image
element itself in a spacer div that adds the extra 8px on each side:

```tsx
<div className="mt-2 mb-2">
  <ImageFull src={imgFoo} alt="..." caption="..." />
</div>
```

`mt-2`/`mb-2` = 8px each; combined with the existing 24px section gap that
gives 32px above and below. Apply this to every image-bearing primitive
placed between text blocks in a new case study, not just the first one.

### Padding inside bespoke cards/blocks

When a case study needs a bespoke card, chip, or table cell that isn't one
of the shared primitives (i.e. it lives in that case study's own `Blocks.tsx`
or inline in `Sections.tsx`), match the padding the shared primitives
already use, don't invent a tighter one. `ProblemGrid`, `StatCards`, and
`NextGrid` (`CaseStudyPrimitives.tsx`) all use `p-6` (24px) for their card
padding — that's the standard. Bespoke cards built at `p-3.5`/`p-4`/`p-5`
or table cells at `px-3 py-3` read as visibly tighter than the rest of the
page and stand out as inconsistent. Default new bespoke card-style
components to `p-6`; only go tighter (`p-4`–`p-5`) for genuinely compact
elements like small status chips or dense table rows, and even then don't
go below `p-4`.

### Brand logo in the overview block

`overviewLogo` renders inside a fixed `size-[48px]` rounded box
(`CaseStudyTemplate.tsx`). A raw brand mark SVG (e.g. via the shared `Logo`
component, `src/app/components/Logo/Logo.tsx`) often has no internal
padding and reads as cramped sitting edge-to-edge in that box, next to the
client name/category text. Wrap it in a badge filling the box with the
brand's primary color as background (the same color as `THEME_PRESETS[<theme>].primaryColor`
in `Theming.tsx`) and size the logo down so it has breathing room inside:

```tsx
overviewLogo={
  <div className="w-full h-full flex items-center justify-center" style={{ background: "#0053e2" }}>
