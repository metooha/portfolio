...
import {THEME_PRESETS, THEME_FONT_CONFIG, getTheme, setTheme, getMegaMode, setMegaMode, MEGA_CHANGE_EVENT} from '../utils/themeManager';
import themeIconMap from '../fonts/theme-icon-map.json';
import {navigationSections} from './pages';
...
export default function AppNav({currentRoute, navigate, isDrawer = false, isOpen = false, onClose}: AppNavProps) {
  const [searchQuery, setSearchQuery] = React.useState('');
  const [activeTheme, setActiveTheme] = React.useState(() => getTheme());
  const activePreset = THEME_PRESETS[activeTheme];
  ...
  React.useEffect(() => {
    const onThemeChange = (e: Event) => {
      ...
      if (name && THEME_PRESETS[name]) setActiveTheme(name);
    };
  ...
  // Filter navigation sections by search query
    ...
    >
      ...
      <div style={{ padding: '20px 16px 0', flexShrink: 0 }}>
        <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <img
            src={`${import.meta.env.BASE_URL}ld-logo.png`}
            alt=""
            width={34}
            height={34}
            style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            ...
            <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--ld-primitive-color-gray-110, #686a70)', letterSpacing: '1.5px', textTransform: 'uppercase' as const, lineHeight: 1.2 }}>
              Portable Kit
            </span>
          </div>
        </div>
      ...
      <div style={{
        ...
        {filteredSections.map((section, sectionIndex) => {
          ...
          return (
            <div key={section.id} style={{ marginTop: sectionIndex === 0 ? '12px' : '0', marginBottom: sectionIndex < filteredSections.length - 1 ? '20px' : '12px' }}>
              <button
                type="button"
        ...
        <div style={{ padding: '14px 16px 16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', marginTop: '8px', fontSize: '11px', lineHeight: 1.2 }}>
            <div style={{ display: 'flex', gap: '6px' }}>
              ...
              <span style={{ color: '#2E2F32', fontWeight: 500 }}>{FONT_DISPLAY_NAMES[THEME_FONT_CONFIG[activeTheme as keyof typeof THEME_FONT_CONFIG]?.textFont] ?? THEME_FONT_CONFIG[activeTheme as keyof typeof THEME_FONT_CONFIG]?.textFont}</span>
            </div>
            <div style={{ display: 'flex', gap: '6px' }}>
              ...
              <span style={{ color: '#2E2F32', fontWeight: 500 }}>{themeIconMap.fonts[themeIconMap.themes[activeTheme as keyof typeof themeIconMap.themes] as keyof typeof themeIconMap.fonts]?.label}</span>
            </div>
          </div>
        </div>
      </div>
