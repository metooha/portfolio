# Handoff checklist: `oportun-ds` → portfolio
...
- [ ] Create `src/app/components/case-studies/<slug>/OportunDsCaseStudy.tsx` (or merge exported code into that path).
...
- [ ] **`routing: special`**: set `PageComponent` on the config entry to `<OportunDsCaseStudy />` (mirror id `"1"` + `WmDesignSystemCaseStudy`).
