...
- [ ] Create `src/app/components/case-studies/airtable-sot/AirtableCaseStudy.tsx` (or merge exported code into that path).
...
- [ ] **`routing: special`**: set `PageComponent` on the config entry to `<AirtableCaseStudy />` (mirror id `"1"` + `WmDesignSystemCaseStudy`).
