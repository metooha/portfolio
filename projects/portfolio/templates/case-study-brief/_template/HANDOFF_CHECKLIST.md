...
- [ ] Create `src/imports/__COMPONENT_NAME__.tsx` (or merge exported code into that path).
...
- [ ] **`routing: special`**: add a branch in `case-study-page.tsx` that returns `<__COMPONENT_NAME__ />` for this `id` (mirror `id === "1"`).
