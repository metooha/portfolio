 ...
 *  built-in base theme (Walmart by default). Each theme is built from a
...
import type { ThemeName } from './Theming';
...
export const SEEDS: SeedDef[] = [
  {
    id: 'primary',
    label: 'Primary / Brand',
    description: 'Brand & action color — primary buttons, links, switches, focus rings, brand text.',
    components: ['Button', 'Link', 'Switch', 'Checkbox', 'Side Nav', 'Focus Ring'],
    default: '#0053e2',
    derive: (hex) => {
  ...
  },
  {
    id: 'navigation',
    label: 'Navigation',
    description: 'Top navigation bar background and the text/icons that sit on it.',
  ...
  },
  {
    id: 'surface',
    label: 'Surface',
    description: 'Page and card background — the base canvas behind everything.',
  ...
  },
  {
    ...
    description: 'Success states — confirmations, positive badges, tags and metrics.',
  ...
  },
  {
    ...
    description: 'Errors and destructive actions — error text, negative badges and borders.',
  ...
  },
  {
    ...
    description: 'Caution and alert states — warning badges, banners and highlights.',
...
export function resolveOverrides(theme: Pick<CustomTheme, 'seeds' | 'tokenOverrides'>): Record<string, string> {
...
export const DEFAULT_BASE_THEME: ThemeName = 'Walmart';
...
export function getCustomTheme(name: string): CustomTheme | undefined {
  return getCustomThemes().find((t) => t.id === name || t.name === name);
}

export function isCustomThemeName(name: string): boolean {
  return getCustomThemes().some((t) => t.id === name || t.name === name);
}
...
/** Inject a theme's resolved overrides into a single :root <style> element. */
export function injectCustomOverrides(theme: Pick<CustomTheme, 'seeds' | 'tokenOverrides'>): void {
