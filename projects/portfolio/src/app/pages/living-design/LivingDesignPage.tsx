...
} from '@/living-design/components/DefaultBreakpoint/DefaultBreakpoint';
import {LivingDesignProvider} from '@/living-design/components/LivingDesign/LivingDesign';
import {useSnackbar} from '@/living-design/components/Snackbar/Snackbar';
import {useAnnounce} from '@/living-design/components/A11yAnnouncement/A11yAnnouncement';
import {ExampleSection, GuidelinesSection, PageWrapper} from './shared';

const SETUP_CODE = `import {LivingDesignProvider} from '@walmart/livingdesign-react';

export default function App() {
  return (
    <LivingDesignProvider defaultBreakpoint="medium">
      <Routes />
...
const HOOKS = [
  ...
  {
    name: 'useLivingDesignContext',
    purpose: 'Returns the active LivingDesign context value; rarely needed directly.',
  },
];
  ...
  return (
    <div style={{display: 'grid', gap: 8}}>
      ...
      <Caption as="p" color="subtle" style={{marginTop: 8}}>
        LivingDesignProvider wraps these four nested providers. You only render this one.
      </Caption>
    </div>
  );
}

export default function LivingDesignPage() {
  return (
    <PageWrapper
      title="Living Design Provider"
      category="System"
      description="Living Design Provider is the single utility component that wires up every cross-cutting React context the kit needs at the root of your app — SSR, accessibility live announcements, default responsive breakpoint, and snackbar. Mount it once at the top of your tree and consume the contexts via hooks below."
    >
      <ExampleSection
        title="What it composes"
        description="LivingDesignProvider wraps four nested providers. You only need to render this one — the rest are wired up internally."
      >
      ...
      >
        ...
        <Caption as="p" color="subtle" style={{marginTop: 12}}>
          Only render <code style={inlineCodeStyle}>LivingDesignProvider</code> once. Mounting two triggers an invariant warning to prevent context conflicts.
        </Caption>
      </ExampleSection>
      ...
      <ExampleSection
        ...
        description="Once LivingDesignProvider is mounted, any descendant can call useSnackbar().addSnack() — no extra wiring needed."
      >
        <LivingDesignProvider defaultBreakpoint="large">
          <SnackbarTrigger />
        </LivingDesignProvider>
      </ExampleSection>
