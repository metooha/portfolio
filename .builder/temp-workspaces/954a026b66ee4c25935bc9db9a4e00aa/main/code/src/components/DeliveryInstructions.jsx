function DeliveryInstructions({ selected, onSelect }) {
  return (
    <div className="px-4 py-4 border-b border-gray-100">
      <h3 className="text-sm font-bold text-walmart-text mb-4">Delivery instructions</h3>
      
      <div className="flex gap-4 mb-4">
        <button
          onClick={() => onSelect('leave')}
          className={`flex-1 py-3 px-4 rounded border-2 text-sm font-medium transition-all ${
            selected === 'leave'
              ? 'border-walmart-blue bg-white text-walmart-text'
              : 'border-walmart-border bg-white text-walmart-text'
          }`}
        >
          Leave at my door
        </button>
      ...
      <div className="flex items-start gap-2 text-xs text-walmart-subtle">
